...
func (_m *PipesStorage) LoadStatus(workspaceID int, sid integration.ID, pid integration.PipeID) (*domain.Status, error) {
	ret := _m.Called(workspaceID, sid, pid)

	var r0 *domain.Status
	if rf, ok := ret.Get(0).(func(int, integration.ID, integration.PipeID) *domain.Status); ok {
		r0 = rf(workspaceID, sid, pid)
	} else {
		if ret.Get(0) != nil {
